...
The instrumentation layer replaces eight global functions---grouped into four categories (\texttt{fetch}; timers \texttt{setTimeout}/\texttt{setInterval}/\texttt{clearTimeout}/\texttt{clearInterval}; event listeners \texttt{addEventListener}/\texttt{removeEventListener}; and \texttt{WebSocket})---with instrumented versions. Each replacement function:
...
lane & string & Execution category: \texttt{ui}, \texttt{api}, \texttt{auth}, \texttt{ws}, \texttt{worker}, or custom \\
beat & number & High-resolution timestamp via \texttt{performance.now()} \\
ts & number & Wall-clock timestamp via \texttt{Date.now()} \\
public & boolean & Whether the event is retained in production builds \\
...
source & string? & Origin: \texttt{auto}, \texttt{manual}, or \texttt{scope} \\
...
\textbf{Pattern 3: Double-Trigger.} The detector identifies pairs of start events with matching labels where the second fires before the first's corresponding end event. For the finding to be reported, both events must target the same operation (same label). End-event classification prefers the structured \texttt{kind} field (\texttt{response}, \texttt{timer-end}, \texttt{timer-tick}, \texttt{timer-clear}, \texttt{error}, \texttt{listener-remove}) set by the auto-instrumentation layer; when \texttt{kind} is absent (manual pulses), the detector falls back to a 14-suffix label allowlist: \texttt{end}, \texttt{complete}, \texttt{response}, \texttt{done}, \texttt{fire}, \texttt{clear}, \texttt{tick}, \texttt{cancel}, \texttt{abort}, \texttt{close}, \texttt{stop}, \texttt{finish}, \texttt{resolve}, \texttt{unsubscribe}. The severity is \texttt{critical} when the two starts carry identical parameters after stripping instrumentation-internal keys (\texttt{generation}, \texttt{latestGeneration}) and \texttt{info} when parameters differ, because different parameters usually indicate intentional concurrency.
...
This adds approximately 25 lines of code to \texttt{instrument.ts} (a module-level \texttt{endpointGeneration} map, two helper functions, and the call-site integrations in the \texttt{fetch} patch) and zero runtime overhead beyond a \texttt{Map.get}/\texttt{Map.set} pair per request. It does not require Babel plugins, React internals, or framework-specific hooks---it operates entirely within the existing global function interception layer.
...
\textbf{What the detectors actually fired.} Across all 77 tests, the analyser produced 193 findings spanning 4 of the 7 detection patterns:
...
\midrule
after-teardown   & 88 & Auto-fires on any timer/listener/fetch event after \texttt{scope.end()} \\
...
\textbf{Why the 11 missed tests are missed.} Nine of the 11 misses (BUG-4, -33, -36, -37, -38, -39, -47, -48, -50) involve concurrent fetches to \emph{distinct URL endpoints}. Examples: an SWR cache-key change from \texttt{/user/alice} to \texttt{/user/bob}; a TanStack Query mutation to \texttt{/todos/update} racing a refetch to \texttt{/todos}; a form's async validation to \texttt{/validate/email} racing a submit to \texttt{/submit}. The response-reorder and stale-overwrite detectors group requests by normalised endpoint label, so they correctly do not flag these as ``the same operation racing itself.'' Detecting cross-endpoint races requires a higher-level abstraction (e.g., key-aware cache observers like the ones SWR and React Query implement internally) that PulsCheck currently does not provide.

The remaining two misses have different causes. BUG-5 (pmndrs/zustand) is a \texttt{setTimeout}-on-unmount pattern whose reproduction uses \texttt{clearTimeout} as a test-safety call before invoking \texttt{analyze()}; the trace therefore contains a valid completion event that the dangling-async detector accepts, and the audit records a miss. This is a reproduction artefact rather than a detector gap---in real application code where \texttt{clearTimeout} is the missing cleanup, the same detector would fire. BUG-41 (axios/axios) is a same-path different-parameter case: both requests hit \texttt{/axios/search} with different \texttt{?q=} query strings, and since \texttt{truncateUrl} uses \texttt{URL.pathname} they group as one endpoint. The double-trigger detector does flag the overlap but downgrades the severity to \texttt{info} with the summary ``different parameters---likely intentional concurrency.'' The audit harness counts only \texttt{warning} or \texttt{critical} verdicts as detection, so BUG-41 is recorded as missed despite producing an informational finding. Only the nine distinct-endpoint cases represent a genuine capability gap in the current detector surface.
...
\textbf{Heuristic detection.} The seven patterns are heuristic, not formally verified. False positives are possible when the detected pattern is intentional behaviour (e.g., deliberate duplicate requests for redundancy). False negatives are possible for race condition patterns not covered by the seven detectors. The 85.7\% detection rate in the real-code audit means 14.3\% of real race conditions are missed. Nine of the 11 misses involve concurrent fetches to distinct URL endpoints, which requires cross-endpoint cache awareness that PulsCheck does not currently provide. The remaining two have separate causes: a reproduction artefact where test-safety \texttt{clearTimeout} is called before \texttt{analyze()} (BUG-5), and an info-severity downgrade on same-path different-parameter concurrency (BUG-41). Only the distinct-endpoint category represents a genuine detector capability gap.
