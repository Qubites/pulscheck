...
Ships a single cleanup-aware AST rule — `fetch-no-abort-in-effect` — which catches `fetch()` inside `useEffect` / `useLayoutEffect` / `useInsertionEffect` without an `AbortController` wired into cleanup. The equivalent `setTimeout` / `setInterval` / `addEventListener` rules already live in `@eslint-react/eslint-plugin`, so we don't duplicate them.
...
Events are stored in a ring buffer with a default capacity of 10,000 events (O(1) insertion, configurable via `registry.configure({ maxTrace })`). Four heuristic detectors run over the trace on an interval (default 5,000 ms) via the built-in reporter. Findings are structurally deduplicated by a fingerprint of pattern, sorted labels, and call site — so one logical bug produces one report, not one per occurrence.
