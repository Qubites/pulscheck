...
// src/dom.ts
var ATTR_PULSE = "data-tw-pulse";
var ATTR_LANE = "data-tw-lane";
var ATTR_MAX = "data-tw-max";
var ATTR_PUBLIC = "data-tw-public";
var ATTR_DOC = "data-tw-doc";
function pulseElement(el) {
  const label = el.getAttribute(ATTR_PULSE);
  if (!label) return;
  const opts = {
    lane: el.getAttribute(ATTR_LANE) ?? "ui",
    public: el.getAttribute(ATTR_PUBLIC) === "true",
    doc: el.getAttribute(ATTR_DOC) ?? void 0,
    maxMs: el.hasAttribute(ATTR_MAX) ? parseInt(el.getAttribute(ATTR_MAX), 10) : void 0,
    meta: { element: el.tagName.toLowerCase(), id: el.id || void 0 }
  };
  tw.pulse(label, opts);
}
...
function printFindings(findings) {
  ...
  console.log(`
\u{1F50D} Found ${findings.length} potential issue(s):
`);
...
function createTracker(options = {}) {
  ...
  function compare(findings) {
    ...
    for (const [fp, tracked2] of Object.entries(state.findings)) {
      if (!currentFingerprints.has(fp) && tracked2.status !== "resolved" && tracked2.status !== "ignored") {
        tracked2.status = "resolved";
        tracked2.resolvedAt = { commit, timestamp: ts, beatRange: [0, 0] };
        tracked2.streak = 0;
        tracked2.history.push({ action: "resolved", commit, timestamp: ts });
      }
    }
    state.lastRun = ts;
  ...
  function ignore(fp) {
    ...
    tracked.history.push({ action: "ignored", commit, timestamp: now2() });
    return true;
  }
  function unignore(fp) {
    const tracked = state.findings[fp];
    if (!tracked || tracked.status !== "ignored") return false;
    tracked.status = "recurring";
    tracked.history.push({ action: "unignored", commit, timestamp: now2() });
    return true;
  }
  function save() {
  ...
  function printDiff(diff, log = console.log) {
    ...
    if (diff.regressed.length > 0) {
      log("  \u{1F504} REGRESSED (was fixed, came back)");
    ...
    if (diff.ignored.length > 0) {
      log(`  \u{1F507} IGNORED (${diff.ignored.length})`);
...
export { VERSION, analyze, createReporter, createTracker, devMode, fingerprint2 as fingerprint, initDomPulse, instrument, observeDom, printFindings, registry, restore, scanDom, tw };
